...
std::string bot::command::Who::execute(const CommandParams& params, const Dependencies& deps)
{
  if (params.args.empty()) return util::emptyArgs().data();

  return deps.jsonUtils->pickRandomChatUser(params.peer_id) + ' ' + params.args + '.';
}
