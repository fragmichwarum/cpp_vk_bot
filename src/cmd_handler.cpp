...
Cmd_traits::cmds_t const Cmd_handler::vk_cmds =
{
  ...
  { "+скачать", { "скачать файл в указанный путь",            &Cmd_handler::download_cmd,    creator } }
};
...
static void excerption_gen(bot::Vk_api& api, Cmd_handler& handler, const string& text, long peer_id) {
  if (split(text).size() > 1) {
    api.send_message(handler.complete_cmd({ text, peer_id }), peer_id);
  }
}

void Cmd_handler::init_cmds(const json& update)
{
  ...
  if (rand() % 100 < 7 && message.at(0) != '+') {
    excerption_gen(_api, *this, message, peer_id);
  }
